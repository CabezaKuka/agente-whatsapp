...
const FOLLETOS = {
  ...
  tempwifi:      'https://raw.githubusercontent.com/CabezaKuka/agente-whatsapp/main/fichaTEMPWIFI.png',
};
...
- HIWIFI vs TEMPWIFI — son dos productos distintos, NUNCA los confundas: el HiWIFI mide temperatura Y humedad (650 Bs). El TempWIFI mide SOLO temperatura, sin humedad (580 Bs). Si el cliente necesita humedad, ofrecele el HiWIFI, no el TempWIFI.
- TEMPWIFI — gancho de primer contacto: distinguí entre dos casos. CASO 1 — saludo genérico sin pregunta concreta: usá siempre este gancho: "Hola 👋 TempWIFI es un monitor de temperatura ambiente que avisa por Telegram si algo sale del rango. Se conecta al WiFi del lugar y ves todo desde cualquier navegador. El equipo cuesta 580 Bs. ¿En qué ambiente lo usaría?" CASO 2 — el primer mensaje ya contiene una pregunta concreta: respondé directamente esa pregunta incluyendo el precio (580 Bs) en la misma respuesta, sin usar el gancho estándar. REGLA DURA: el texto que escribís antes de [FOLLETO_tempwifi] SIEMPRE tiene que incluir el precio (580 Bs).
...
FOLLETOS-IMAGENES DISPONIBLES — solo estas palabras clave existen, no inventés otras:
...
async function procesarMensajes(from) {
  ...
  try {
    ...
    } else if (reply.includes('[FOLLETO_tempwifi]')) {
      const texto = reply.replace('[FOLLETO_tempwifi]', '').trim();
      if (texto) { const r = await enviarMensaje(from, texto); saveOutgoing({ waId: from, text: texto, metaMessageId: extractMetaMessageId(r), status: 'sent' }); }
      const r2 = await enviarImagen(from, FOLLETOS.tempwifi);
      saveOutgoing({ waId: from, text: '[Imagen: folleto tempwifi]', metaMessageId: extractMetaMessageId(r2), status: 'sent' });
    } else {
