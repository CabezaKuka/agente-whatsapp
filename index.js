...
- HIWIFI — gancho de primer contacto: la primera vez que el cliente pregunta precio, pide más información, o pide la ficha del HiWIFI, usá siempre exactamente este mensaje: "Hola 👋 HiWiFi monitorea la temperatura y humedad y avisa si los valores salen del rango configurado. El equipo cuesta 650 Bs. ¿Dónde necesita controlar la temperatura y humedad?" — podés variar únicamente la pregunta final pero siempre en esa misma forma natural, nunca con lista de opciones y nunca con tuteo. REGLA DURA: el texto que escribís antes de [FOLLETO_hiwifi] SIEMPRE tiene que incluir el precio (650 Bs) — está PROHIBIDO mandar [FOLLETO_hiwifi] con un texto que no mencione el precio. NO uses la frase "sin instalar nada". No menciones Telegram, PDF, CSV, IA, gráficos de 7/30 días ni "en tiempo real" en ese primer contacto — eso va recién si el cliente sigue la conversación.
...
- LOGGER — gancho de primer contacto: la primera vez que el cliente pregunta precio, pide más información, o pide la ficha del Logger, usá siempre exactamente este mensaje: "Hola 👋 El HiWIFI Logger registra la temperatura y humedad y guarda todo el historial, sin necesitar internet. El equipo cuesta 495 Bs. ¿Dónde necesita registrar la temperatura y humedad?" — podés variar únicamente la pregunta final pero siempre en forma natural, nunca con lista de opciones y nunca con tuteo. REGLA DURA: el texto que escribís antes de [FOLLETO_logger] SIEMPRE tiene que incluir el precio (495 Bs). No menciones detalles técnicos adicionales en ese primer contacto — eso va recién si el cliente sigue la conversación.
  ...
  if (!diasOrdenados.length) {
    ...
    <div class="day-group">
      ...
      for (const chat of chatsDelDia) {
        ...
        html += `
        <a class="chat-row${pausados.has(chat.wa_id) ? ' pausado' : ''}" href="/chat/${encodeURIComponent(chat.wa_id)}">
          ...
          <div class="chat-time">${pausados.has(chat.wa_id) && chat.last_direction === 'in' ? '⏸️🔴 ' : pausados.has(chat.wa_id) ? '⏸️ ' : ''}${escapeHtml(hora)}</div>
        </a>`;
      }
      html += `</div></div>`;
    }
...
<html><head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .pause-btn{padding:10px;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;margin-bottom:8px;width:100%;transition:background .2s}
    .pause-btn.activo{background:#25D366;color:#fff}
    .pause-btn.pausado{background:#e53935;color:#fff}
  </style>
</head>
<body>
  <div class="header">
